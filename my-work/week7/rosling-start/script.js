...
function gotData(incomingData) {
  ...
  // continent
  let continents = incomingData.reduce(function (acc, d, i) {
  ...
  function drawViz() {
    ...
    let dataGroups = vizGroup.selectAll('.dataGroups').data(currentYearData, (d) => d.Country);

    // take care of entering elements
    let enteringGroups = dataGroups.enter().append('g').attr('class', 'dataGroups');
    ...
    enteringGroups.append('text')
      ...
      .attr('font-size', '1em')
      .attr('fill', 'black');
    enteringGroups.attr('transform', getIncomingLocation)
      .transition().duration(INTERVAL).ease(d3.easeLinear)
      .attr('transform', getGroupLocation)

    // take care of updating elements
    dataGroups.transition().duration(INTERVAL).ease(d3.easeLinear)
      .attr('transform', getGroupLocation)
    dataGroups.select('circle')
      .transition().duration(INTERVAL).attr('r', d => rScale(d.pop))
